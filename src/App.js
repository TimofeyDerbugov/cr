...
function nextId(table) {
  const ids = db.get("nextId", { orders: 1, finances: 1, warehouse: 1 });
...
const initialPrices = {
  ...
  gates: { "Нет": 0, "Эконом": 8000, "Стандарт": 14000, "Премиум": 22000 },
  wickets: { "Нет": 0, "Эконом": 4500, "С замком": 7500 },
};
...
function Btn({ children, onClick, variant = "primary", size = "md", style = {}, disabled = false }) {
  const base = { borderRadius: 8, fontFamily: "'Golos Text',sans-serif", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 6, cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.5 : 1, transition: "all 0.15s", border: "none", fontSize: size === "sm" ? 12 : size === "lg" ? 15 : 13, padding: size === "sm" ? "5px 10px" : size === "lg" ? "12px 20px" : "8px 14px" };
  const variants = { primary: { background: ORANGE, color: "#fff" }, ghost: { background: "transparent", color: MUTED, border: `1px solid ${BORDER}` }, danger: { background: DANGER + "22", color: DANGER, border: `1px solid ${DANGER}44` }, success: { background: SUCCESS + "22", color: SUCCESS, border: `1px solid ${SUCCESS}44` }, outline: { background: "transparent", color: ORANGE, border: `1px solid ${ORANGE}` } };
  return <button onClick={onClick} disabled={disabled} style={{ ...base, ...variants[variant], ...style }}>{children}</button>;
}

function Badge({ children, color = ORANGE }) {
  return <span style={{ background: color + "22", color, border: `1px solid ${color}44`, padding: "2px 8px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{children}</span>;
}

function Card({ children, style = {}, onClick }) {
  return (
    <div onClick={onClick} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 12, padding: 16, cursor: onClick ? "pointer" : "default", transition: "border-color 0.2s", ...style }}
...
function Field({ label, children, required }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <label style={{ display: "block", color: MUTED, fontSize: 11, marginBottom: 4 }}>{label}{required && <span style={{ color: ORANGE }}>*</span>}</label>
      {children}
    </div>
  );
}
...
function Modal({ title, children, onClose, width = 600 }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000CC", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 16, width: "100%", maxWidth: width, maxHeight: "90vh", overflow: "auto" }}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <div style={{ fontFamily: "'Unbounded',sans-serif", fontSize: 22, fontWeight: 900, color: ORANGE, lineHeight: 1.1 }}>Заборыч</div>
...
function OrderForm({ order, onSave, onClose, prices }) {
  const [form, setForm] = useState(order || { client: "", phone: "", address: "", fenceType: "3D сетка 1.8", perimeter: "", gates: "Нет", wicket: "Нет", demolition: false, delivery: 0, status: "замер", manager: "Иван П.", note: "" });
      ...
      <div>
        ...
        {est && (
          <div style={{ background: CARD2, borderRadius: 8, padding: 12, border: `1px solid ${ORANGE}44` }}>
            ...
            {[["Материалы", est.totals.materials], ["Работы", est.totals.works], ...(est.totals.gatesCost > 0 ? [["Ворота", est.totals.gatesCost]] : []), ...(est.totals.wicketCost > 0 ? [["Калитка", est.totals.wicketCost]] : [])].map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 3 }}>
                <span style={{ color: MUTED }}>{k}</span><span>{fmt(v)}</span>
              </div>
            ))}
...
function Orders({ orders, setOrders, prices }) {
  ...
  const save = (data) => {
    let updated;
    if (editOrder) {
      updated = orders.map(o => o.id === editOrder.id ? { ...editOrder, ...data } : o);
    } else {
      const today = new Date().toISOString().slice(0, 10);
      updated = [...orders, { ...data, id: nextId("orders"), created: today }];
    }
    setOrders(updated);
  ...
  return (
    <div>
      <SectionTitle action={<Btn onClick={() => { setEditOrder(null); setShowForm(true); }}>+ Новая заявка</Btn>}>Заявки</SectionTitle>
      <div style={{ display: "flex", gap: 6, marginBottom: 16, flexWrap: "wrap" }}>
        {statuses.map(s => (
          <button key={s} onClick={() => setFilter(s)} style={{ padding: "5px 12px", borderRadius: 20, border: `1px solid ${filter === s ? ORANGE : BORDER}`, background: filter === s ? ORANGE + "22" : "transparent", color: filter === s ? ORANGE : MUTED, fontSize: 12, cursor: "pointer", fontFamily: "'Golos Text',sans-serif", fontWeight: filter === s ? 600 : 400 }}>
            {s === "все" ? "Все" : STATUS_ICONS[s] + " " + s}
          </button>
        ))}
      </div>
      {filtered.map(o => (
        ...
        <div>
          ...
          {[["Имя", order.client], ["Телефон", order.phone], ["Адрес", order.address], ["Менеджер", order.manager], ["Дата", order.created]].map(([k, v]) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
              <span style={{ color: MUTED, fontSize: 12 }}>{k}</span><span style={{ fontSize: 12 }}>{v}</span>
            </div>
          ))}
        </div>
        <div>
          ...
          <div style={{ background: CARD2, borderRadius: 8, overflow: "hidden", marginBottom: 12 }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr style={{ background: "#1E1E1E" }}>
                {["Наименование", "Кол-во", "Ед.", "Цена", "Сумма"].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: "left", color: MUTED, fontSize: 11, fontWeight: 600, borderBottom: `1px solid ${BORDER}` }}>{h}</th>)}
              </tr></thead>
              <tbody>
                {est.lines.map((l, i) => (
...
function Calculator({ prices }) {
  const [form, setForm] = useState({ perimeter: 80, fenceType: "3D сетка 1.8", gates: "Нет", wicket: "Нет", demolition: false, delivery: 0 });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
          ...
          {est && (
            <>
              <Card style={{ marginBottom: 12 }}>
                ...
                {[["Столбов", est.specs.poles + " шт"], ["Секций", est.specs.sections + " шт"], ["Трубы 60×40", est.specs.tubes + " м"], ["Щебень", est.specs.gravel + " м³"], ["Саморезы", est.specs.screws + " уп"]].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ color: MUTED, fontSize: 12 }}>{k}</span><span style={{ fontSize: 12, fontWeight: 600 }}>{v}</span>
                  </div>
                ))}
              </Card>
              <Card>
                ...
                {[["Материалы", est.totals.materials], ["Работы", est.totals.works], ...(est.totals.gatesCost > 0 ? [["Ворота", est.totals.gatesCost]] : []), ...(est.totals.wicketCost > 0 ? [["Калитка", est.totals.wicketCost]] : [])].map(([k, v]) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                    <span style={{ color: MUTED, fontSize: 13 }}>{k}</span><span style={{ fontSize: 13 }}>{fmt(v)}</span>
                  </div>
                ))}
...
function Warehouse({ warehouse, setWarehouse }) {
  ...
  const add = () => {
    ...
    const today = new Date().toISOString().slice(0, 10);
    const item = { ...newItem, id: nextId("warehouse"), qty: Number(newItem.qty), minQty: Number(newItem.minQty), lastIn: today };
    const updated = [...warehouse, item];
    setWarehouse(updated);
    db.set("warehouse", updated);
    setNewItem({ name: "", qty: "", unit: "м", minQty: "" });
    setShowAdd(false);
  };

  const adjust = (id, delta) => {
    const updated = warehouse.map(w => w.id === id ? { ...w, qty: Math.max(0, w.qty + delta) } : w);
    setWarehouse(updated);
    db.set("warehouse", updated);
  };
      ...
      {warehouse.map(w => (
        <Card key={w.id} style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ textAlign: "center", minWidth: 60 }}>
                <div style={{ fontWeight: 700, fontSize: 18, color: w.qty <= w.minQty ? DANGER : w.qty <= w.minQty * 1.5 ? WARN : SUCCESS }}>{w.qty}</div>
          ...
          <div style={{ marginTop: 8, background: CARD2, borderRadius: 4, height: 4, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 4, width: `${Math.min(100, (w.qty / (w.minQty * 3)) * 100)}%`, background: w.qty <= w.minQty ? DANGER : w.qty <= w.minQty * 1.5 ? WARN : SUCCESS, transition: "width 0.3s" }} />
          </div>
        </Card>
      ))}
      {showAdd && (
...
function Finances({ finances, setFinances }) {
  ...
  const add = () => {
    ...
    const today = new Date().toISOString().slice(0, 10);
    const rec = { ...newRec, id: nextId("finances"), amount: Number(newRec.amount), date: today };
    const updated = [...finances, rec];
    setFinances(updated);
    db.set("finances", updated);
        ...
        <Card key={e.id} style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 40, height: 40, borderRadius: "50%", background: ROLE_COLORS[e.role] + "33", display: "flex", alignItems: "center", justifyContent: "center", color: ROLE_COLORS[e.role], fontWeight: 700, fontSize: 15 }}>
...
Отвечай кратко, по-русски, профессионально. Помогай с расчётами заборов, анализом данных, рекомендациями по бизнесу.`;
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, system: context, messages: [...history, { role: "user", content: userMsg }] })
      });
  ...
  const suggestions = ["Сколько прибыли в этом месяце?", "Какие материалы заканчиваются?", "Посчитай забор 120м профлист без ворот", "Дай советы по оптимизации склада"];
  ...
  return (
    <div>
        ...
        <div style={{ flex: 1, overflowY: "auto", maxHeight: 350, paddingRight: 4 }}>
          {messages.map((m, i) => (
            <div key={i} style={{ marginBottom: 12, display: "flex", flexDirection: m.role === "user" ? "row-reverse" : "row", gap: 8 }}>
              <div style={{ maxWidth: "80%", padding: "8px 12px", borderRadius: 12, background: m.role === "user" ? ORANGE : CARD2, color: m.role === "user" ? "#fff" : TEXT, fontSize: 13, lineHeight: 1.5, whiteSpace: "pre-wrap", borderTopRightRadius: m.role === "user" ? 4 : 12, borderTopLeftRadius: m.role === "assistant" ? 4 : 12 }}>{m.content}</div>
            </div>
          ))}
          {loading && <div style={{ display: "flex", gap: 8 }}><div style={{ padding: "8px 12px", borderRadius: 12, background: CARD2, fontSize: 13, color: MUTED }}>⏳ Думаю...</div></div>}
          <div ref={bottomRef} />
        </div>
        <div style={{ borderTop: `1px solid ${BORDER}`, paddingTop: 12, marginTop: 8 }}>
          <div style={{ display: "flex", gap: 8 }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && send()} placeholder="Спросите что-нибудь..." style={{ flex: 1 }} />
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        {suggestions.map(s => (
          <button key={s} onClick={() => setInput(s)} style={{ background: CARD2, border: `1px solid ${BORDER}`, color: MUTED, fontSize: 11, padding: "4px 10px", borderRadius: 20, cursor: "pointer", fontFamily: "'Golos Text',sans-serif" }}>{s}</button>
        ))}
      </div>
    </div>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    setFinances(db.get("finances", []));
    setWarehouse(db.get("warehouse", []));
    const saved = db.get("prices", null);
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        <div style={{ width: 220, background: CARD, borderRight: `1px solid ${BORDER}`, display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh" }}>
          ...
          <nav style={{ flex: 1, padding: "12px 8px", overflowY: "auto" }}>
            {NAV.map(n => (
              <button key={n.id} onClick={() => setScreen(n.id)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 12px", borderRadius: 8, border: "none", background: screen === n.id ? ORANGE + "22" : "transparent", color: screen === n.id ? ORANGE : MUTED, fontFamily: "'Golos Text',sans-serif", fontSize: 13, fontWeight: screen === n.id ? 600 : 400, cursor: "pointer", transition: "all 0.15s", marginBottom: 2, textAlign: "left", position: "relative" }}>
                <span style={{ fontSize: 16 }}>{n.icon}</span>
                {n.label}
                {n.id === "warehouse" && lowStockCount > 0 && (
                  <span style={{ marginLeft: "auto", background: WARN, color: "#000", borderRadius: 10, fontSize: 10, fontWeight: 700, padding: "1px 6px" }}>{lowStockCount}</span>
                )}
                {n.id === "orders" && (
                  <span style={{ marginLeft: "auto", background: ORANGE + "33", color: ORANGE, borderRadius: 10, fontSize: 10, fontWeight: 700, padding: "1px 6px" }}>{orders.filter(o => o.status !== "завершено").length}</span>
                )}
              </button>
            ))}
          </nav>
          <div style={{ padding: 12, borderTop: `1px solid ${BORDER}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: ORANGE + "33", display: "flex", alignItems: "center", justifyContent: "center", color: ORANGE, fontWeight: 700, fontSize: 13 }}>ИП</div>
              <div>
